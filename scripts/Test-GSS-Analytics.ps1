...
Assert-Equal $script:GssAnalysisPolicyVersion 'gss-analysis-policy/v4' 'Versioned analysis policy'
...
Assert-Equal ([string]$script:GssAnalysisPolicy.confidence.semantic_label) 'ResponseVolumeTier' 'Response-volume tier is the semantic sample-size label'
Assert-Equal ([string]$script:GssAnalysisPolicy.source_design.population_scores_source) 'rolling_aggregate_workbook' 'Population scores remain aggregate'
Assert-True (-not [bool]$script:GssAnalysisPolicy.source_design.population_raw_rows_available) 'Population raw rows are explicitly unavailable'
Assert-Equal ([string]$script:GssAnalysisPolicy.source_design.row_level_scores_source) 'surveys_with_comments_only' 'Row scores are commenter-only'
Assert-True (-not [bool]$script:GssAnalysisPolicy.source_design.commenter_rows_population_representative) 'Commenter rows are not population representative'
Assert-Equal ([string]$script:GssAnalysisPolicy.source_design.population_driver_modeling_status) 'PopulationRawDataUnavailable' 'Population modeling is unavailable by source design'
Assert-True ([bool]$script:GssAnalysisPolicy.commenter_lens.enabled) 'Commenter lens is enabled'
Assert-True (-not [bool]$script:GssAnalysisPolicy.commenter_lens.population_prevalence_claim_allowed) 'Commenter lens cannot claim population prevalence'
Assert-True (-not [bool]$script:GssAnalysisPolicy.commenter_lens.statistical_significance_claim_allowed) 'Commenter lens cannot claim statistical significance'
Assert-True (-not [bool]$script:GssAnalysisPolicy.commenter_lens.email_attachment_allowed) 'Commenter lens cannot become an email attachment'
Assert-True (-not [bool]$script:GssAnalysisPolicy.commenter_lens.row_level_persistence_allowed) 'Commenter lens cannot persist row data'
Assert-Equal ([string]$script:GssAnalysisPolicy.forecasting.status) 'Deferred' 'Forecasting remains deferred'
$modelingAvailability = Get-GssPopulationModelingAvailability
Assert-Equal $modelingAvailability.Status 'PopulationRawDataUnavailable' 'Population modeling fixed availability status'
Assert-True ([bool]$modelingAvailability.NonBlocking) 'Population modeling availability remains operationally nonblocking'
Assert-Equal $modelingAvailability.ReasonCode 'non_comment_survey_rows_not_provided' 'Population modeling availability reason code'
Assert-True (-not [string]::IsNullOrWhiteSpace([string]$modelingAvailability.Reason)) 'Population modeling availability has a plain-language reason'
...
Assert-Equal $richmondOverall.ResponseVolumeTier 'High' 'Metric response-volume tier'
