...
Assert-Equal $script:GssAnalysisPolicyVersion 'gss-analysis-policy/v3' 'Versioned analysis policy'
...
Assert-Equal ([string]$script:GssAnalysisPolicy.population_export.reconciliation.failure_status) 'DataBlocked' 'Population reconciliation failure is analytically isolated'
Assert-Equal ([double]$script:GssAnalysisPolicy.inference.comparison.false_discovery_rate) 0.10 'Inference false discovery rate'
Assert-Equal ([int]$script:GssAnalysisPolicy.driver_model.validation.bootstrap_repetitions) 1000 'Deterministic bootstrap count'
Assert-Equal ([int]$script:GssAnalysisPolicy.driver_model.validation.random_seed) 20260723 'Deterministic model seed'
Assert-True (-not [bool]$script:GssAnalysisPolicy.driver_model.blocks_workbook_or_backup_or_package) 'Shadow model cannot block operational transaction'
Assert-True (-not [bool]$script:GssAnalysisPolicy.driver_model.email_attachment_allowed_in_shadow) 'Shadow model cannot change the three-attachment package'
Assert-Equal ([string]$script:GssAnalysisPolicy.forecasting.status) 'Deferred' 'Forecasting remains deferred'
$missingPopulationModel = Invoke-GssShadowModelReview -ReviewFolder $env:TEMP -InputPath ''
Assert-Equal $missingPopulationModel.Status 'DataBlocked' 'Missing population export blocks only modeling'
Assert-True ([bool]$missingPopulationModel.NonBlocking) 'Missing population export remains operationally nonblocking'
Assert-Equal $missingPopulationModel.ReasonCode 'population_export_not_provided' 'Missing population export reason code'
